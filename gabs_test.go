...
func TestExistsWithArrays(t *testing.T) {
	...
	if val, err = Parse(sample); err != nil {
		t.Errorf("Failed to parse: %v", err)
		return
	}
...
func TestFindArray(t *testing.T) {
	for i, this := range []struct {
		...
		val, err := Parse([]byte(this.input))
		if err != nil {
			t.Errorf("[%d] Failed to parse: %s", i, err)
			return
		}

		target := val.Get(this.target)
...
func TestDeletes(t *testing.T) {
	...
	if actual := jsonParsed.ToString(); actual != expected {
...
func TestExamples(t *testing.T) {
	...
	value, ok = jsonParsed.Get("outter.inner.value1").Data().(float64)
	if value != 10.0 || !ok {
	...
	value, ok = jsonParsed.Get("does.not.exist").Data().(float64)
	if value != 0.0 || ok {
		t.Errorf("wrong value: %v, %v", value, ok)
	}

	jsonParsed, _ = Parse([]byte(`{"array":[ "first", "second", "third" ]}`))

	expected := []string{"first", "second", "third"}

	children := jsonParsed.S("array").Children()
	if children == nil {
		t.Errorf("Error: %v", ErrNotObjOrArray)
		return
	}
...
func TestExamples2(t *testing.T) {
	...
	if jsonObj.ToString() != expected {
		t.Errorf("Non matched output: %v != %v", expected, jsonObj.ToString())
	}
...
func TestDotNotation(t *testing.T) {
	...
	if result, _ := val.Get("test.inner.value").Data().(float64); result != 10 {
...
func TestModify(t *testing.T) {
	...
	if out := val.ToString(); `{"test":{"value":45},"test2":20}` != out {
		t.Errorf("Incorrectly serialized: %v", out)
	}

	if out := val.Search("test").ToString(); `{"value":45}` != out {
		t.Errorf("Incorrectly serialized: %v", out)
	}
}
...
func TestChildrenMap(t *testing.T) {
	...
	if val := json1.Get("objectOne.num").Data().(int); val != 500 {
...
func TestNestedAnonymousArrays(t *testing.T) {
	...
	if val := json1.Get("array").Index(1).Index(1).Data().(float64); val != 5 {
		t.Errorf("nested child test: %v != %v", val, 5)
	}

	if val := json1.Get("array").Index(3).Index(0).S("test").Data().(float64); val != 50 {
		t.Errorf("nested child object test: %v != %v", val, 50)
	}

	json1.Get("array").Index(3).Index(0).Set(200, "test")

	if val := json1.Get("array").Index(3).Index(0).S("test").Data().(int); val != 200 {
...
func TestArrays(t *testing.T) {
	...
	if englishPlaces := json2.Search("places").Data(); englishPlaces != nil {
		json1.Get("languages.english").Set(englishPlaces, "places")
	} else {
...
func TestArraysRoot(t *testing.T) {
	...
	if expected, actual := `["test1","test2",{"foo":"bar"}]`, val.ToString(); expected != actual {
...
func TestInvalid(t *testing.T) {
	...
	validObj, err := Parse([]byte(`{}`))
	if err != nil {
		t.Errorf("failed to parse '{}'")
	}

	invalidStr := validObj.S("Doesn't exist").ToString()
	if "{}" != invalidStr {
...
func TestNoTypeChildren(t *testing.T) {
	jsonObj, err := Parse([]byte(`{"not_obj_or_array":1}`))
	if err != nil {
		t.Error(err)
	}
	if child := jsonObj.S("not_obj_or_array").Children(); child != nil {
		t.Errorf("Unexpected value returned: children not nil %v", child)
	}
	exp := 0
	if act := jsonObj.S("not_obj_or_array").ChildrenMap(); len(act) != exp {
		t.Errorf("Unexpected value returned: children map should be zero length, act:%v", act)
	}
}

func TestBadIndexes(t *testing.T) {
	jsonObj, err := Parse([]byte(`{"array":[1,2,3]}`))
	if err != nil {
		t.Error(err)
	}
...
func TestDefault(t *testing.T) {
	jsonObj, err := Parse([]byte(`{"key":"value"}`))
	if err != nil {
		t.Error(err)
	}
	actString := jsonObj.Get("key_not_exist").String()
	assert.Equal(t, "", actString)
	actString = jsonObj.Get("key_not_exist").String("default_value")
	assert.Equal(t, "default_value", actString)

	actFloat64 := jsonObj.Get("key_not_exist").Float64()
	assert.Equal(t, 0.00, actFloat64)
	actFloat64 = jsonObj.Get("key_not_exist").Float64(1.1)
	assert.Equal(t, 1.1, actFloat64)

	actInt64 := jsonObj.Get("key_not_exist").Int64()
	assert.Equal(t, int64(0), actInt64)
	actInt64 = jsonObj.Get("key_not_exist").Int64(1)
	assert.Equal(t, int64(1), actInt64)

	actBool := jsonObj.Get("key_not_exist").Bool()
	assert.Equal(t, false, actBool)
	actBool = jsonObj.Get("key_not_exist").Bool(true)
	assert.Equal(t, true, actBool)
}

func TestConversionFromString(t *testing.T) {
	jsonObj, err := Parse([]byte(`{"test_int64":"123", "test_float64":"123.456"}`))
	if err != nil {
		t.Error(err)
	}

	actInt64 := jsonObj.Get("test_int64").Int64()
	assert.Equal(t, int64(123), actInt64)
	actFloat64 := jsonObj.Get("test_float64").Float64()
	assert.Equal(t, float64(123.456), actFloat64)
}
